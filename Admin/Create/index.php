...
$errmsg="";
//send verification email to create new user
...
if (isset($_POST["submit2"])){
	//echo "asd";
	//checks if all input fields are full if true then create a new user and insert into database
	if(is_full()){
			...
			if(filter_var($_POST["email"], FILTER_VALIDATE_EMAIL)){
					...
					//if a unique username continue to insert new user and insert into database
					if(!$dublicate){
						$pass= generate(8);
						$password=  password_hash($pass, PASSWORD_BCRYPT);
						$vtoken = bin2hex(openssl_random_pseudo_bytes(64));
						$sql=("INSERT INTO Users (`Role`,`Username` ,`password` ,`Email`, `Name`,`Address`, `Province` ,`City`,`Country`,`Postal`,`vtoken`) VALUES('2','".$_POST['username']."','".$password."','".$_POST['email']."','".$_POST['Name']."','".$_POST['street']."','".$_POST['province']."','".$_POST['city']."','".$_POST['country']."','".$_POST['postal']."','".$vtoken."')");
						...
						if($connection->query($sql)){
							...
							$sql= ("INSERT INTO Admins (`USERID`,`SignatureID#`,`Name`) VALUES('".$userid."','".$sig."','".$_POST['Name']."')");
							if($connection->query($sql)){
...
/**
 * returns a boolean if the form input is completely filled
 * @return boolean true or false if the form is complete
 */
function is_full(){
	$GLOBALS['errmsg'];
